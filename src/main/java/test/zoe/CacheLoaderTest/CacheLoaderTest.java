...
public class CacheLoaderTest {
    ...
    public void testWeight() throws ExecutionException, InterruptedException {
        //如果不同的高速缓存条目有不同的“权重”，例如，如果你的缓存值有着完全不同的记忆的足迹--
        // 你可以用cachebuilder指定一个权重函数。
    ...
    public void testLoadNullValueUseOptional() {
        ...
        java.util.Optional<Employee> employeeOptional = cache.getUnchecked("guava");
        if (employeeOptional.isPresent()) {
            Employee employee = employeeOptional.get();
            System.out.println("employee：" + employee.toString());
        }

        java.util.Optional<Employee> employeeNull = cache.getUnchecked("null");
        if (!employeeNull.isPresent()) {
            System.out.println("employee is null");
            Employee def = cache.getUnchecked("null").orElse(new Employee("default", "default", "default"));
